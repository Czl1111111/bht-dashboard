...
function genDefaults(isMonth,wd){
  ...
  add(items,'urgent',isMonth?'月度-紧急':'周度-紧急',urgent);
  add(items,'important',isMonth?'月度-重要':'周度-重要',important);
  var pfx=isMonth?'月度':'周度';
  items.push({id:++id,text:pfx+' 整体目标: 利润率达8% (当前'+wd.total.margin.toFixed(1)+'%)',priority:'urgent',children:[
...
function initDragDrop(){
  ...
  if(!container) return;
  var wraps=container.querySelectorAll('.section-wrap');
  var dragged=null;
  wraps.forEach(function(w){
    ...
    w.addEventListener('dragover',function(e){e.preventDefault();this.classList.add('drag-over')});
    ...
    w.addEventListener('drop',function(e){
      ...
      if(dragged!==this){
        ...
        if(e.clientY<mid){container.insertBefore(dragged,this)}
  ...
  if(wkId.indexOf('month_')!==0){
    ...
    if(wkIdx>0){
      for(var i=wkIdx-1;i>=0;i--){
        ...
        if(pwd&&pwd.skus&&pwd.skus.length){window._prevWd=pwd;break;}
      }
    }
  }
  ...
  <div class="section-wrap" draggable="true" data-section="todos">
    ...
    <div class="section-body" id="todosBody">
      <div style="padding:4px 14px 12px">
        <div class="todo-wrap"><h4>📅 周度待办 (基于当前周数据分析)</h4><div id="weekTodoFilters" class="todo-filters"></div><div id="weekTodoList" class="todo-list"></div><div class="todo-add-wrap"><button id="btn-add-week-todo">+ 添加周度待办</button></div>
        </div>
        <div class="todo-wrap" style="margin-top:18px"><h4>📊 月度待办 (基于月度数据分析8月规划)</h4><div id="monthTodoFilters" class="todo-filters"></div><div id="monthTodoList" class="todo-list"></div><div class="todo-add-wrap"><button id="btn-add-month-todo">+ 添加月度待办</button></div></div>
        </div>
      </div>
    </div>
  </div>
